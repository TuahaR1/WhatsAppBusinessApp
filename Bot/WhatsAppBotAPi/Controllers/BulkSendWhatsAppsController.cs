...
using whatsappbotApi.Controllers;
...
namespace WhatsAppBotAPi.Services.Controllers
{
	public class BulkSendWhatsAppsController : ControllerBase
    {
		...
		public BulkSendWhatsAppsController(ILogger logger,IWhatsAppBussinesManager whatsAppBussinesManager ,IWhatsAppBusinessClient whatsAppBusinessClient, IWebHostEnvironment environment, IConfiguration configuration)
        {
            _logger = logger;
        ...
        public async Task<ActionResult> ReadAndTraverseCSV(FileInfo fileInfo)
        {
            try
            {
                ...
                SendMessageController sendMessageController = new(_configuration, _whatsAppBusinessClient,_whatsAppBussinesManager);
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    wUpContact.WupNum = CommonHelper.PrepNumber(row["WUp Num"].ToString());    // Prep the number and return the prepped number
                    ...
                    {// Text messages is a simple send (with or without [params or templates]) NO attachments
                        WAMIds = CommonHelper.GetWAMId((await sendMessageController.SendWhatsApp_TextAsync(sendWhatsAppPayload)).Value);
                    }
                    ...
                    {// NOT a template do a normal Media send (Audio, Doc, Image, Video)
                        WAMIds = CommonHelper.GetWAMId((await sendMessageController.SendWhatsApp_MediaAsync(sendWhatsAppPayload)).Value);
                    ...
                    { // This is (Doc, Image, Video) WITH a template, cater for different calls
                        ...
                        switch (wUpContact.MsgType.Type)
                        {
                            ...
                            case enumMessageType.Image:
                                WAMIds = CommonHelper.GetWAMId((await sendMessageController.SendWhatsApp_TemplateImage_ParameterAsync(sendWhatsAppPayload)).Value);
                                break;

                            case enumMessageType.Video:
                                WAMIds = CommonHelper.GetWAMId((await sendMessageController.SendWhatsApp_TemplateVideo_ParameterAsync(sendWhatsAppPayload)).Value);
                                break;
                        }
                    }
